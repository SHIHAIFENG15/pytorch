...
这个测试是否只验证 Python 逻辑、规则是否写对”，不真的依赖 GPU、通信或分布式运行？如果是，可以保持普通 TestCase、CPU、fake 或 meta 写法,不需要强行改成 accelerator 测试。
这个测试是否创建真实 DTensor、DeviceMesh等，如果是，优先让它使用当前测试设备,而不是写死"cuda"。
这个测试是不是专门验证某个后端独有能力，比如 XLA、NCCL、CUDA profiler、CUDA Graph、Triton 或某个特定 backend？如果是，保留专属 class 或专属 skip。
常见的 2 个现有基础设施：
...
源码片段：torch/testing/_internal/distributed/_tensor/common_dtensor.py
 ...
 def device_type(self) -> str:
     if (
         ...
         return DEVICE_TYPE
​
不同情况的结果：当测试环境没有可用 accelerator，或设备数量少于 self.world_size，返回 "cpu"；否则返回当前 accelerator 类型。具体 accelerator 类型由同文件中 DEVICE_TYPE = torch.accelerator.current_accelerator().type 决定，见 common_dtensor.py。
...
文件：test/distributed/tensor/test_dtensor_dispatch_overhead.py
...
文件：test/distributed/tensor/test_dtensor_logging.py
...
文件：test/distributed/tensor/test_dtensor_compile.py
...
文件：test/distributed/tensor/test_dtensor_ops.py
