...
@unittest.skipIf(not is_dynamo_supported(), "Dynamo not supported")
class TestMoveToDevicePass(TestCase):
    def test_move_device_to(self, device):
        ...
        class M(torch.nn.Module):
            def forward(self, x):
                x = torch.ops.aten.to.device(
                    x, device=device0, dtype=torch.float32
                )
                return x + x
...
instantiate_device_type_tests(TestMoveToDevicePass, globals(), except_for="cpu")


@unittest.skipIf(not is_dynamo_supported(), "Dynamo not supported")
class TestMoveToDevicePassRNN(TestCase):
    # Exclude privateuse1: NPU fp32 GRU fails after move_to_device_pass (DynamicGRUV2).
...
instantiate_device_type_tests(
    TestMoveToDevicePassRNN, globals(), except_for=("cpu", "privateuse1")
)
