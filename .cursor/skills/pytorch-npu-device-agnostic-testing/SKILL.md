...
Dynamo / Inductor **源码**里的 `DeviceInterface` 注册表、`GPU_TYPES`、`has_triton()`、
以及 docs2 表 **C1–C8 / 特性解耦 / 分布式图模式** 属于 RFC #189138，用 skill
`rfc-189138-compile-device-interface`。不要和本 skill 的测试改造混在一个 PR 里。
...
- **容器**：`pytorch-npu-yxz-dev`（CANN 8.5.0 + Python 3.11 + 8×910B3）。所有 venv/编译/测试**必须在容器里跑**，宿主机看不到容器内解释器。 <!-- codespell:ignore -->
...
- CANN `set_env.sh` 在 `set -u` 下有 unbound variable，`source` 前后用 `set +eu` / `set -eu` 包住。 <!-- codespell:ignore -->
...
CPU 版编译进 `venv_cpu`，NPU 版（torch_npu）编译进 `venv_npu`。编译前需 `source` CANN 的 `set_env.sh`（脚本已处理）。 <!-- codespell:ignore -->
